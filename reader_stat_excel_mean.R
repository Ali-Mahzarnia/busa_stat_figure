...
for (i in 48:(dim(master)[2]-1)) {
  ...
  summary(lm)
  write.xlsx2(as.data.frame(  anova(lm)), paste0("mean.xlsx"), sheet =colnames(master)[i],append = TRUE)
  eta=effectsize::cohens_f(lm, alternative='two.sided')
  write.xlsx2(eta, paste0("mean.xlsx"), sheet =paste0(colnames(master)[i],"_cohen_"),append = TRUE)
  ...
  write.xlsx2(posthoc2, paste0("mean.xlsx"), sheet =paste0(colnames(master)[i],"_emmeans_"),append = TRUE)
